...
public class BulletPool : MonoBehaviour
{
    ...
    private List<Bullet> flameThrowerBullets = new List<Bullet>();
    ...
    void TakeBulletFromPool(int bulletIndex, Vector3 spawnPosition, Quaternion bulletRotation, Vector2 bulletDirection)
    {
        ...
        if (bulletIndex == 3)
        {
            for (int i = 0; i < flameThrowerBullets.Count; i++)
            {
                if (!flameThrowerBullets[i].gameObject.activeInHierarchy)
                {
                    flameThrowerBullets[i].gameObject.SetActive(true);
                    flameThrowerBullets[i].gameObject.transform.position = spawnPosition;
                    flameThrowerBullets[i].gameObject.transform.rotation = bulletRotation;
                    flameThrowerBullets[i].MoveInDirection(bulletDirection);
    ...
    void CreateNewBullet(int bulletIndex, Vector3 spawnPosition, Quaternion bulletRotation, Vector2 bulletDirection)
    {
        ...
        if (bulletIndex == 3)
        {
            flameThrowerBullets.Add(newBullet.GetComponent<Bullet>());
        }
